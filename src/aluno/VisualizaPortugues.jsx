...
import { Typography, Box, List, ListItem, Modal, Button } from '@mui/material';
...
export function VisualizaPortugues() {
    ...
    // Função para buscar avisos e conteúdos da API
    const fetchData = async () => {
        const token = localStorage.getItem('jwtToken');
        try {
            const avisosResponse = await axios.get('http://127.0.0.1:5000/listar-avisos/portugues', {
                headers: { Authorization: `Bearer ${token}` },
            });
            setAvisos(avisosResponse.data.avisos);

            const conteudosResponse = await axios.get('http://127.0.0.1:5000/listar-conteudos/portugues', {
                headers: { Authorization: `Bearer ${token}` },
            });
...
export default VisualizaPortugues;

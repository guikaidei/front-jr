...
export function VisualizaNaturais() {
    ...
    const fetchData = async () => {
        const token = localStorage.getItem('jwtToken'); // Autenticação
        try {
            const avisosResponse = await axios.get('http://127.0.0.1:5000/listar-avisos/naturais', {
                headers: { Authorization: `Bearer ${token}` },
            });
            setAvisos(avisosResponse.data.avisos);

            const conteudosResponse = await axios.get('http://127.0.0.1:5000/listar-conteudos/naturais', {
                headers: { Authorization: `Bearer ${token}` },
            });
...
export default VisualizaNaturais;
